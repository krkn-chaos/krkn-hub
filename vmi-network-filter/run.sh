...
envsubst < $KRAKEN_FOLDER/config/config.yaml.template > $KRAKEN_FOLDER/config/vmi-network-filter-config.yaml
...
if [[ $KRKN_DEBUG == "True" ]];then
  ...
  cat $KRAKEN_FOLDER/config/vmi-network-filter-config.yaml
fi

python3.11 run_kraken.py --config=$KRAKEN_FOLDER/config/vmi-network-filter-config.yaml $extra_var
